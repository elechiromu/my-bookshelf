          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '14px', display: 'none' }}>{user.displayName}</span>
              </div>
              <button
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={getMonthlyStats(statsYear)} margin={{ top: 30, right: 10, left: 10, bottom: 5 }}>
                    ...
                    <YAxis 
                      ...
                      label={{ value: '(冊)', position: 'top', offset: 10, fontSize: 12, fill: '#9ca3af' }}
                    />
                    <Bar dataKey="count" radius={[4, 4, 0, 0]} maxBarSize={40}>
                      <LabelList 
                        dataKey="count" 
                        position="top" 
                        style={{ fontSize: '12px', fill: '#4b5563', fontWeight: '600' }}
...
function SearchResultCard({ result, onAdd, onCancel }) {
  ...
  return (
    <div style={{
      ...
      <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#4b5563', marginBottom: '16px' }}>
        検索結果
      </h3>
          ...
          {isEditMode ? (
            ...
            <>
              ...
              <p style={{ fontSize: '14px', color: '#6b7280', marginBottom: '16px' }}>
                {editedBook.author}
                {editedBook.publisher && ` / ${editedBook.publisher}`}
              </p>
            </>
          )}
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              {[1, 2, 3, 4, 5].map(star => (
                ...
                >
                  <Star
                    size={28}
                    fill={star <= editedBook.rating ? '#fbbf24' : 'none'}
                    stroke={star <= editedBook.rating ? '#fbbf24' : '#d1d5db'}
                    strokeWidth={1.5}
                  />
                </button>
              ))}
